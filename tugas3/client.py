...
#variabel global simpen kunci
my_rsa_private_key = None
my_rsa_public_key = None
shared_des_key = None
des_key_lock = threading.Lock() #mengamankan akses ke shared_des_key

def send_pickled(sock, data_dict):
    sock.sendall(pickle.dumps(data_dict)) #kirim data via pickle
    ...
    while True:
        try:
            ...
            if msg_type == 'get_key_request':
                ...
                #kirim public key ke peminta (client A) via server
                response = {
                    'type': 'key_response',
                    'target': message['from'],
            ...
            #balesan public key dari c 
                ...
                encrypted_des_key = message['payload']
                
                #dekripsi dgn private key RSA
            ...
            #Penanganan Chat (habis kunci DES ada)
            elif msg_type == 'chat':
                print(f"\n[CLIENT] Menerima pesan chat terenkripsi...")
                with des_key_lock:
                    if shared_des_key:
                        try:
                            pesan_terdekripsi = decrypt_des(message['payload'], shared_des_key)
